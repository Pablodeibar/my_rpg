...
int *event_clicked(global *global, rect_t *rect, attack_t *attack, int *stat)
{
    ...
    for (int i = 0; i != 4; i++) {
        x1 = rect[i + 2].pos.x;
        y1 = rect[i + 2].pos.y;
        if (mu_x > x1 && mu_x < x1 + rect[i + 2].size.x &&
        mu_y > y1 && mu_y < y1 + rect[i + 2].size.y &&
        attack->competence_use[i] != -1 && attack->mana_rest -
        attack->mana_cost_use[i] >= 0) {
...
int reward(monster_t *monster, character_t *character)
{
    if (monster->hp <= 0) {
