...
int *stat)
{
    ...
    attack_t attack;
    sfTexture *texture = sfTexture_createFromFile("assets/images/sw.png", NULL);
    sfSprite *sprite = create_background(texture);
    ...
    && character->hp > 0 && monster->hp > 0) {
        ...
        sfRenderWindow_drawSprite(global->window, sprite, sfFalse);
    ...
    global->screen = 1;
    reward(monster, character);
    destroy_scene_fight(sprite, rect, texture);
    free(rect);
}
...
void is_there_fight(global *global, character_t *character, monster_t *monster)
{
    ...
    if (global->screen == 2 || global->screen == 1) {
